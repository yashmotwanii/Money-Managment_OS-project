...
void make_req(char * body) {
    ...
    strcpy(global_req.user1,uname1);
    strcpy(global_req.user2,uname2);
    global_req.amount=atoi(amt);
    global_req.type=1;
    printf("global_req : %s %s %d %d END \n",global_req.user1,global_req.user2,global_req.amount,global_req.type);

    add_request(global_req);
}
// ///////////////////
